...
export default function AdminDashboard() {
  ...
  const [newPresentation, setNewPresentation] = useState({
    title: '', presenter_name: '', description: '',
  });
  ...
  async function createRoom() {
    ...
    const { error: rError } = await supabase
      .from('rooms')
      .insert({ presentation_id: pData.id });

    if (!rError) {
      setNewPresentation({ title: '', presenter_name: '', description: '' });
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '40px 32px' }}>
        ...
        {showNewForm && (
          <div className="card fade-up" style={{ marginBottom: 32, padding: 28, borderColor: 'rgba(232,52,28,0.25)' }}>
            <h2 style={{ fontSize: 17, marginBottom: 20, fontFamily: 'var(--font-sans), sans-serif', fontWeight: 500 }}>
              새 발표 + 설문 방 생성
            </h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <InputField
                  ...
                  onChange={(v) => setNewPresentation(p => ({ ...p, title: v }))}
                  placeholder="AI가 바꾸는 미래 직업"
                />
                <InputField
                  label="발표자 이름 *"
                  value={newPresentation.presenter_name}
                  onChange={(v) => setNewPresentation(p => ({ ...p, presenter_name: v }))}
                  placeholder="김민준"
                />
              </div>
              <InputField
                ...
                onChange={(v) => setNewPresentation(p => ({ ...p, description: v }))}
                placeholder="발표 내용을 간략하게 설명해주세요."
              />
            </div>
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button
                className="btn btn-primary"
                onClick={createRoom}
                disabled={creating || !newPresentation.title || !newPresentation.presenter_name}
                style={{ fontSize: 14 }}
              >
                {creating ? '생성 중...' : '생성하기'}
              </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 32 }}>
          {[
            { label: '전체 발표', value: rooms.length },
            { label: '진행중', value: rooms.filter(r => r.is_open).length },
            { label: '결과 공개', value: rooms.filter(r => r.is_published).length },
          ].map((stat) => (
            <div key={stat.label} style={{
              ...
              <p style={{ fontSize: 12, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>
                {stat.label}
              </p>
        ...
        {loading ? (
          ...
          <div className="card" style={{ textAlign: 'center', padding: '60px', color: 'var(--text3)' }}>
            <p>발표가 없습니다. 위 버튼으로 발표를 추가해주세요.</p>
          </div>
        ) : (
...
function AdminRoomCard({
  ...
  return (
    <div className="card" style={{
      ...
      justifyContent: 'space-between', gap: 16, padding: '18px 24px',
    }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          {room.is_open ? (
            ...
            <span className="badge badge-gold">공개됨</span>
          ) : (
            <span className="badge badge-dim">종료</span>
          )}
          {responseCount !== null && (
        ...
        <h3 style={{
          fontSize: 16, fontFamily: 'var(--font-serif), serif',
    ...
    <div>
      ...
      <input
        ...
        style={{
          ...
          color: 'var(--text)', fontSize: 14, padding: '10px 12px',
